...
const Sidebar: React.FC<SidebarProps> = ({ godModeActive }) => {
  return (
      ...
      <div className="p-4 lg:p-6 pb-2 border-b border-white/5">
        <div className="flex items-center gap-4 mb-2 lg:mb-6 justify-center lg:justify-start">
          <div className="relative group cursor-pointer">
            <div
              className={`bg-center bg-no-repeat bg-cover rounded-none clip-path-hexagon size-10 lg:size-12 border transition-all duration-300 group-hover:scale-105 ${
                godModeActive
                  ? 'border-secondary/50 shadow-neon-purple'
                  : 'border-primary/50 shadow-neon-blue'
              }`}
              style={{
                backgroundImage: `url("https://lh3.googleusercontent.com/aida-public/AB6AXuCqOGvnWeMYYBAF2WkK0YfteHXGGMLo0e0xozmV9d7Ay5EW_2gx4aRUNHA5yPbfJQ3VVmr0ixCtOH7BBkWruXKvhuCG81faeSsoYXcPS7gJYSINUrxNXXjXfJC5KtuXHZeku_HauGm2uYWQWks2FYjQf-NB9MEpxrvzxShFsZ3aYkQlIuVVGSg2uXCXTb-iBwsdKXEit8NKPy7VetVtCGPOHmYweEeYXsf8sdD9BAWjWrZgojAhwgK7zc9FhhZ9XJ9hcGzC7hwM3sKT")`,
              }}
            ></div>
            <div className={`absolute -inset-1 blur opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-full ${
                godModeActive ? 'bg-secondary/20' : 'bg-primary/20'
              }`}
            ></div>
          </div>
          <div className="hidden lg:flex flex-col">
            <h1 className="text-white text-lg font-bold tracking-widest uppercase font-mono holographic-text">
              God Mode
            </h1>
            <p className={`text-[10px] font-bold tracking-[0.2em] animate-pulse ${
                godModeActive ? 'text-secondary' : 'text-primary'
              }`}
            >
              {godModeActive ? 'PRIVILEGES ESCALATED' : 'SYSTEM_ACTIVE'}
            </p>
          </div>
        </div>
      </div>
      <div className="flex flex-col gap-2 p-2 lg:p-4 flex-1 overflow-y-auto font-mono">
        <p className="hidden lg:block px-3 text-[10px] font-bold text-slate-600 uppercase tracking-widest mb-2 mt-2">
          Neural Link
        </p>
        <a
          className="flex items-center gap-3 px-3 py-3 rounded-sm bg-primary/5 border-l-2 border-primary text-primary group transition-all hover:bg-primary/10 hover:translate-x-1"
          href="#"
        >
        ...
        <a
          className="flex items-center gap-3 px-3 py-3 rounded-sm hover:bg-white/5 border-l-2 border-transparent hover:border-white/20 text-slate-500 hover:text-white group transition-all hover:translate-x-1"
          href="#"
        >
          ...
          <div className="relative">
            <div className="size-8 lg:size-10 rounded-full bg-slate-800 flex items-center justify-center overflow-hidden border border-white/10 relative z-10">
              <img
                ...
                src="https://lh3.googleusercontent.com/aida-public/AB6AXuAguTHHvY_A_EdTngUflhxqiV-Yu3w6NGIxlBZTWP0hqZKrKKNf-Uhs_4bmiRv51iYCiUmbx9hM2ErrxWn7Z8wIdSqLWdw3RBq8w22dR7rvK7ij1coh6FgOga_abZtC6G8G5mua9ZaHNQFjO8qORyjulhGRVe7khb6Mlc2d6-CfOPAPKxQCa4nGgz84TX0MPAPa9dAVQniTodlH9lyBv8efojoi6JRW6FyX6WXCM-IYJhsUF6YfvnptsGUEuppwlkQOfHMOgjWQOJuF"
              />
            </div>
            <div className={`absolute -inset-0.5 rounded-full blur-sm transition-all ${
                godModeActive ? 'bg-secondary/30 animate-pulse-fast' : 'bg-accent/30'
              }`}
            ></div>
            <div className={`absolute bottom-0 right-0 size-2.5 rounded-full border-2 border-black z-20 transition-all ${
                godModeActive ? 'bg-secondary shadow-[0_0_8px_#7000ff]' : 'bg-accent shadow-[0_0_8px_#00ff9d]'
              }`}
            ></div>
          </div>
          <div className="hidden lg:flex flex-col">
            ...
            <span className={`text-[10px] font-mono tracking-tight transition-colors ${
                godModeActive ? 'text-secondary' : 'text-slate-500'
              }`}
            >
              {godModeActive ? 'GOD MODE ACCESS' : 'LVL 5 ACCESS'}
            </span>
          </div>
